...
export const THEORY_formSchema = z.object({
    TaForms: z.array(
        z.object({
            ...
            taOtherWorkDay: z.string({ required_error: "โปรดเลือกวันปฏิบัติงานอื่น" }),
            taOtherWorkDayStart: z.string({ required_error: "โปรดกรอกเวลาเริ่มต้น" }).datetime({ message: "โปรดกรอกเป็นเวลา ISO" }),
            taOtherWorkDayEnd: z.string({ required_error: "โปรดกรอกเวลาสิ้นสุด" }).datetime({ message: "โปรดกรอกเป็นเวลา ISO" }),
        })
    )
})
