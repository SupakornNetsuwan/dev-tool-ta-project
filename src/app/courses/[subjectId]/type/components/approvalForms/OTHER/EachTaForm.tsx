...
const Content: React.FC<{ index: number }> = ({ index }) => {
  const { register } = useFormContext<OTHERFormType>();

  return (
    <div className="flex-1 border border-l-0 p-4">
      <div className="grid grid-cols-2 gap-2">
        <FieldWrapper
          label={<Label.Root>ที่</Label.Root>}
      ...
      <div className="grid grid-cols-2 gap-2">
        <FieldWrapper
          label={<Label.Root>คุณวุฒิ</Label.Root>}
          errorComponent={<ShowInputError inputName={`TaForms.${index}.taCertificate`} />}
        >
          <input
            type="text"
            {...register(`TaForms.${index}.taCertificate`)}
            className="w-full justify-self-end rounded border p-2 text-gray-500 outline-none disabled:opacity-50"
          />
        </FieldWrapper>
        <FieldWrapper
          label={<Label.Root>วันปฏิบัติงาน</Label.Root>}
        ...
        <FieldWrapper
          label={<Label.Root>ภาระงานที่มอบหมาย</Label.Root>}
          errorComponent={<ShowInputError inputName={`TaForms.${index}.taTask`} />}
        >
          <input
            type="text"
            {...register(`TaForms.${index}.taTask`)}
            className="w-full justify-self-end rounded border p-2 text-gray-500 outline-none disabled:opacity-50"
          />
        </FieldWrapper>
      </div>
