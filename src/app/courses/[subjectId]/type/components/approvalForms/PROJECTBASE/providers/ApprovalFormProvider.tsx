...
import { PROJECTBASE_formSchema } from "@/app/api/subjects/[subjectId]/approvalForm/func/PROJECTBASE/PROJECTBASE_formSchema";
...
/**
 * @description สำหรับฟอร์มขออนุมัติ TA ประเภท PROJECTBASE
 */
...
const ApprovalFormProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  const methods = useForm<PROJECTBASEFormType>({
    resolver: zodResolver(PROJECTBASE_formSchema),
    values: {
      TaForms: data?.data.data?.ProjectBaseForm.map((item) => stringifiedObj(item)) || [],
    },
    defaultValues: {
      TaForms: [],
    },
  ...
  const onSubmit: SubmitHandler<PROJECTBASEFormType> = (payload) => {
    updateOrCreate.mutate(
      { ...payload, approvalFormType: "PROJECTBASE" },
      {
