...
import { LT_EIGHT_formSchema } from "@/app/api/subjects/[subjectId]/approvalForm/func/LT_EIGHT/LT_EIGHT_formSchema";
...
/**
 * @description สำหรับฟอร์มขออนุมัติ TA ประเภท LT_EIGHT
 */
...
const ApprovalFormProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  const methods = useForm<LTFormType>({
    resolver: zodResolver(LT_EIGHT_formSchema),
    values: {
      TaForms: data?.data.data?.LTForm.map((item) => stringifiedObj(item)) || [],
    },
    defaultValues: {
      TaForms: [],
    },
  ...
  const onSubmit: SubmitHandler<LTFormType> = (payload) => {
    // console.log(payload)
    updateOrCreate.mutate(
      { ...payload, approvalFormType: "LT_EIGHT" },
      {
