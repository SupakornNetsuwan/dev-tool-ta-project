...
const DetailCourseComponent: React.FC<{ course: Course }> = ({ course }) => {
  ...
  const updatedCourse = useUpdateCourse(course.subjectId);
  // fix 2 - ทำไมถึงใช้ Tanstack query แล้วยังจะไปใช้ useSate ตรง professorList อีก
  const getUsers = useGetUsers();
  // fix 3 - ตรง useMemo อันนี้สร้างมาทำไม เพราะมันไม่จำเป็นเพียงแค่เพื่อนำไปใช้เป็นสมาชิกของ Dependency array
  ...
  // fix 1 - ทำไมไม่ filter จาก server! หรือ ไม่ก็ filter จาก Tanstack ไปเลย ก็สร้าง query ใหม่สำหรับดึง professor
      ...
      <div className="relative flex  flex-col  overflow-hidden bg-gray-50  ">
        <div className="flex items-center justify-center">
          <div className="w-8/12 bg-white p-2 ">
            <p className="text-sx font-medium text-blue-500">ข้อมูลรายวิชา</p>
              ...
              <div className="px-4 py-4 sm:grid sm:grid-cols-3 sm:gap-80 sm:px-0">
                ...
                <dd className="mt-1 text-sm leading-6 text-gray-700 sm:col-span-2 sm:mt-0">
                  <SelectProfessorComponent
                    ...
                    selectedProfessor={couresProfessor}
                  ></SelectProfessorComponent>
                </dd>
              </div>
              <div className="px-4 py-4 sm:grid sm:grid-cols-3 sm:gap-80 sm:px-0">
